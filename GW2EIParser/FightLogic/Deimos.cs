...
namespace GW2EIParser.Logic
{
    public class Deimos : RaidLogic
    {
        ...
        protected override HashSet<ushort> GetUniqueTargetIDs()
        {
            ...
            {
                (ushort)ParseEnum.NPCIDs.Deimos,
                (ushort)Thief,
                (ushort)Drunkard,
                (ushort)Gambler,
            };
        ...
        public override void SpecialParse(FightData fightData, AgentData agentData, List<CombatItem> combatData)
        {
            ...
            // Find target
            NPC target = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Deimos);
            if (target == null)
            {
        ...
        public override List<PhaseData> GetPhases(ParsedLog log, bool requirePhases)
        {
            ...
            NPC mainTarget = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Deimos);
            if (mainTarget == null)
            {
                throw new InvalidOperationException("Main target of the fight not found");
            }
        ...
        public override void ComputeNPCCombatReplayActors(NPC npc, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (npc.ID)
            {
                case (ushort)ParseEnum.NPCIDs.Deimos:
        ...
        {
            return HPBasedCM(combatData, (ushort)ParseEnum.NPCIDs.Deimos, 40e6);
        }
    }
}
