...
namespace GW2EIParser.Logic
{
    public class Sabetha : RaidLogic
    {
        ...
        public override List<PhaseData> GetPhases(ParsedLog log, bool requirePhases)
        {
            ...
            NPC mainTarget = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Sabetha);
            if (mainTarget == null)
            {
        ...
        protected override List<ushort> GetFightNPCsIDs()
        {
            ...
            {
                (ushort)ParseEnum.NPCIDs.Sabetha,
                (ushort)Kernan,
                (ushort)Knuckles,
                (ushort)Karde,
                (ushort)BanditSapper,
        ...
        public override void ComputeNPCCombatReplayActors(NPC target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.NPCIDs.Sabetha:
                    ...
                    foreach (AbstractCastEvent c in flameWall)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, width, height, initialDirection, width / 2, (start, start + preCastTime), "rgba(255, 100, 0, 0.2)", new AgentConnector(target)));
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, width, height, initialDirection, width / 2, 360, (start + preCastTime, start + preCastTime + duration), "rgba(255, 50, 0, 0.5)", new AgentConnector(target)));
                        }
                    }
                    break;
                case (ushort)Kernan:
                ...
                case (ushort)Karde:
                    ...
                    foreach (AbstractCastEvent c in flameBlast)
                    {
                        ...
                        if (facing != null)
                        {
                            replay.Actors.Add(new PieDecoration(true, 0, radius, facing, 60, (start, end), "rgba(255,200,0,0.5)", new AgentConnector(target)));
                        }
                    }
                    break;
                default:
        ...
        public override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in timedBombs)
            {
                int start = (int)c.Time;
                int end = start + 3000;
                replay.Actors.Add(new CircleDecoration(false, 0, 280, (start, end), "rgba(255, 150, 0, 0.5)", new AgentConnector(p)));
                replay.Actors.Add(new CircleDecoration(true, end, 280, (start, end), "rgba(255, 150, 0, 0.5)", new AgentConnector(p)));
            }
            // Sapper bombs
            ...
            foreach (AbstractBuffEvent c in sapperBombs)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(false, 0, 180, (sapperStart, sapperEnd), "rgba(200, 255, 100, 0.5)", new AgentConnector(p)));
