...
namespace GW2EIParser.Logic
{
    public class TwinLargos : RaidLogic
    {
        ...
        protected override List<ushort> GetFightNPCsIDs()
        {
            ...
            {
                (ushort)ParseEnum.NPCIDs.Kenut,
                (ushort)ParseEnum.NPCIDs.Nikare
            };
        }
        ...
        public override List<PhaseData> GetPhases(ParsedLog log, bool requirePhases)
        {
            ...
            NPC nikare = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Nikare);
            if (nikare == null)
            {
                throw new InvalidOperationException("Nikare not found");
            }
            phases[0].Targets.Add(nikare);
            NPC kenut = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Kenut);
            if (kenut != null)
            {
        ...
        public override void ComputeNPCCombatReplayActors(NPC target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.NPCIDs.Nikare:
                    //CC
                    ...
                    break;
                case (ushort)ParseEnum.NPCIDs.Kenut:
                    //CC
                    ...
                    foreach (AbstractCastEvent c in boonSteal)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            replay.Actors.Add(new RotatedRectangleDecoration(false, 0, width, height, rotation, width / 2, (start + delay, start + delay + duration), "rgba(255, 175, 0, 0.8)", new AgentConnector(target)));
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, width, height, rotation, width / 2, (start + delay, start + delay + duration), "rgba(255, 175, 0, 0.2)", new AgentConnector(target)));
                        }
                    }
                    break;
                default:
        ...
        public override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in waterToDrop)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(false, 0, debuffRadius, (toDropStart, toDropEnd), "rgba(255, 100, 0, 0.4)", new AgentConnector(p)));
                    replay.Actors.Add(new CircleDecoration(true, toDropStart + timer, debuffRadius, (toDropStart, toDropEnd), "rgba(255, 100, 0, 0.4)", new AgentConnector(p)));
                    ...
                    {
                        replay.Actors.Add(new CircleDecoration(true, toDropStart + duration, radius, (toDropEnd, toDropEnd + duration), "rgba(100, 100, 100, 0.3)", new InterpolatedPositionConnector(poisonPrevPos, poisonNextPos, toDropEnd), debuffRadius));
                        replay.Actors.Add(new CircleDecoration(false, toDropStart + duration, radius, (toDropEnd, toDropEnd + duration), "rgba(230, 230, 230, 0.4)", new InterpolatedPositionConnector(poisonPrevPos, poisonNextPos, toDropEnd), debuffRadius));
                    }
                }
            }
            ...
            foreach (AbstractBuffEvent c in bubble)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(true, 0, radius, (bubbleStart, bubbleEnd), "rgba(0, 200, 255, 0.3)", new AgentConnector(p)));
                }
            }
        }
        ...
        {
            return HPBasedCM(combatData, (ushort)ParseEnum.NPCIDs.Nikare, 18e6);
        }
    }
}
