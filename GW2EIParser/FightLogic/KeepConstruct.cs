            ...
            switch (npc.ID)
            {
                case (ushort)ParseEnum.NPCIDs.KeepConstruct:
                    ...
                    foreach (AbstractBuffEvent c in kcOrbCollect)
                    {
                        ...
                        {
                            kcOrbEnd = (int)c.Time;
                            replay.Decorations.Add(new CircleDecoration(false, 0, 300, (kcOrbStart, kcOrbEnd), "rgba(255, 0, 0, 0.5)", new AgentConnector(npc)));
                            replay.Decorations.Add(new CircleDecoration(true, kcOrbEnd, 300, (kcOrbStart, kcOrbEnd), "rgba(255, 0, 0, 0.5)", new AgentConnector(npc)));
                        }
                    }
                    ...
                    foreach (AbstractCastEvent c in blades1)
                    {
                        ...
                        replay.Decorations.Add(new CircleDecoration(true, 0, 200, (start, start + (ticks + 1) * 1000), "rgba(255,0,0,0.4)", new AgentConnector(npc)));
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, facing, 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        for (int i = 1; i < ticks; i++)
                        {
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(facing.Y, facing.X) * 180 / Math.PI + i * 360 / 8), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                        }
                    }
                    foreach (AbstractCastEvent c in blades2)
                    {
                        ...
                        replay.Decorations.Add(new CircleDecoration(true, 0, 200, (start, start + (ticks + 1) * 1000), "rgba(255,0,0,0.4)", new AgentConnector(npc)));
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, facing, 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI), 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        for (int i = 1; i < ticks; i++)
                        {
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(facing.Y, facing.X) * 180 / Math.PI + i * 360 / 8), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI + i * 360 / 8), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                    ...
                    foreach (AbstractCastEvent c in blades3)
                    {
                        ...
                        replay.Decorations.Add(new CircleDecoration(true, 0, 200, (start, start + (ticks + 1) * 1000), "rgba(255,0,0,0.4)", new AgentConnector(npc)));
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI), 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI + 120), 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI - 120), 360 * 3 / 32, (start, start + 2 * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts twice as long
                        for (int i = 1; i < ticks; i++)
                        {
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI + i * 360 / 8), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI + i * 360 / 8 + 120), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                            replay.Decorations.Add(new PieDecoration(true, 0, 1600, (int)Math.Round(Math.Atan2(-facing.Y, -facing.X) * 180 / Math.PI + i * 360 / 8 - 120), 360 * 3 / 32, (start + 1000 + i * duration, start + 1000 + (i + 1) * duration), "rgba(255,200,0,0.5)", new AgentConnector(npc))); // First blade lasts longer
                        }
                    }
                    // phantasms locations
                ...
                case (ushort)Ianim:
                    replay.Decorations.Add(new CircleDecoration(false, 0, 600, (crStart, crEnd), "rgba(255, 0, 0, 0.5)", new AgentConnector(npc)));
                    replay.Decorations.Add(new CircleDecoration(true, 0, 400, (crStart, crEnd), "rgba(0, 125, 255, 0.5)", new AgentConnector(npc)));
                    break;
                case (ushort)GreenPhantasm:
                    int lifetime = 8000;
                    replay.Decorations.Add(new CircleDecoration(true, 0, 210, (crStart, crStart + lifetime), "rgba(0,255,0,0.2)", new AgentConnector(npc)));
                    replay.Decorations.Add(new CircleDecoration(true, crStart + lifetime, 210, (crStart, crStart + lifetime), "rgba(0,255,0,0.3)", new AgentConnector(npc)));
                    break;
