...
namespace GW2EIParser.Logic
{
    public class Cairn : RaidLogic
    {
        ...
        public override void ComputeTargetCombatReplayActors(Target target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.EvtcTargetIDS.Cairn:
                    ...
                    foreach (AbstractCastEvent c in swordSweep)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, width, height, initialDirection, width / 2, (start, start + preCastTime), "rgba(200, 0, 255, 0.1)", new AgentConnector(target)));
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, width, height, initialDirection, width / 2, (start + preCastTime, start + preCastTime + initialHitDuration), "rgba(150, 0, 180, 0.5)", new AgentConnector(target)));
        ...
        public override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in agony)
            {
                ...
                replay.Actors.Add(new CircleDecoration(false, 0, 220, (agonyStart, agonyEnd), "rgba(255, 0, 0, 0.5)", new AgentConnector(p)));
            }
        }
