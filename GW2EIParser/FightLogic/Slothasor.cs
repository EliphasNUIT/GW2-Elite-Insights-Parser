...
namespace GW2EIParser.Logic
{
    public class Slothasor : RaidLogic
    {
        public Slothasor(ushort triggerID) : base(triggerID)
        {
            ...
            new EnemyBoonRemoveMechanic(34467, "Narcolepsy", new MechanicPlotlySetting("diamond-tall","rgb(255,0,0)"), "CC Fail","Narcolepsy (Failed CC)", "CC Fail",0, (br,log) => br.RemovedDuration > 120000),
        ...
        protected override List<ushort> GetFightNPCsIDs()
        {
            ...
            {
                (ushort)ParseEnum.NPCIDs.Slothasor,
                (ushort)Slubling1,
                (ushort)Slubling2,
                (ushort)Slubling3,
                (ushort)Slubling4
        ...
        public override List<PhaseData> GetPhases(ParsedLog log, bool requirePhases)
        {
            ...
            NPC mainTarget = NPCs.Find(x => x.ID == (ushort)ParseEnum.NPCIDs.Slothasor);
            if (mainTarget == null)
            {
        ...
        public override void ComputeNPCCombatReplayActors(NPC target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.NPCIDs.Slothasor:
                    ...
                    foreach (AbstractCastEvent c in breath)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            int angle = 60;
                            replay.Actors.Add(new PieDecoration(true, 0, range, direction, angle, (start, start + preCastTime), "rgba(255,200,0,0.1)", new AgentConnector(target)));
                    ...
                    foreach (AbstractCastEvent c in shakes)
                    {
                        int start = (int)c.Time;
                        int end = start + c.ActualDuration;
                        replay.Actors.Add(new CircleDecoration(false, 0, 700, (start, end), "rgba(255, 0, 0, 0.4)", new AgentConnector(target)));
                        replay.Actors.Add(new CircleDecoration(true, end, 700, (start, end), "rgba(255, 0, 0, 0.4)", new AgentConnector(target)));
                    }
                    break;
                default:
                    break;
        ...
        public override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in poisonToDrop)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(false, 0, 180, (toDropStart, toDropEnd), "rgba(255, 255, 100, 0.5)", new AgentConnector(p)));
                    replay.Actors.Add(new CircleDecoration(true, toDropStart + 8000, 180, (toDropStart, toDropEnd), "rgba(255, 255, 100, 0.5)", new AgentConnector(p)));
                    ...
                    {
                        replay.Actors.Add(new CircleDecoration(true, toDropStart + 90000, 900, (toDropEnd, toDropEnd + 90000), "rgba(255, 0, 0, 0.3)", new InterpolatedPositionConnector(poisonPrevPos, poisonNextPos, toDropEnd), 180));
                    }
                }
            }
            // Transformation
            ...
            foreach (AbstractBuffEvent c in slubTrans)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(true, 0, 180, (transfoStart, transfoEnd), "rgba(0, 80, 255, 0.3)", new AgentConnector(p)));
                }
            }
            // fixated
            ...
            foreach (AbstractBuffEvent c in fixatedSloth)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(true, 0, 120, (fixatedSlothStart, fixatedSlothEnd), "rgba(255, 80, 255, 0.3)", new AgentConnector(p)));
                }
            }
        }
    }
