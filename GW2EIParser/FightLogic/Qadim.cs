...
namespace GW2EIParser.Logic
{
    public class Qadim : RaidLogic
    {
        ...
        public Qadim(ushort triggerID) : base(triggerID)
        {
            ...
            new HitOnPlayerMechanic(53153, "Fiery Dance", new MechanicPlotlySetting("asterisk-open","rgb(255,100,0)"), "Fiery Dance", "Fiery Dance (Fire running along metal edges)", "Fire on Lines", 0),
            new HitOnPlayerMechanic(52383, "Fiery Dance", new MechanicPlotlySetting("asterisk-open","rgb(255,100,0)"), "Fiery Dance", "Fiery Dance (Fire running along metal edges)", "Fire on Lines", 0),
            ...
            new HitOnPlayerMechanic(52820, "Fire Wave", new MechanicPlotlySetting("circle-open","rgb(255,100,0)"), "Qadim Wave","Fire Wave (Shockwave after Qadim's Mace attack)", "Mace Shockwave",0),
            new HitOnPlayerMechanic(52224, "Fire Wave", new MechanicPlotlySetting("circle-open","rgb(255,100,0)"), "Destr Wave","Fire Wave (Shockwave after Destroyer's Jump or Stomp)", "Destroyer Shockwave",0),
            ...
            new HitOnPlayerMechanic(53013, "Fireball", new MechanicPlotlySetting("circle-open","rgb(255,200,0)",10), "Hydra FBall","Fireball (Hydra)", "Hydra Fireball",0),
            ...
            new HitOnPlayerMechanic(52705, "Tail Swipe", new MechanicPlotlySetting("diamond-open","rgb(255,200,0)"), "Wyv Pizza","Wyvern Tail Swipe (Pizza attack)", "Tail Swipe",0),
        ...
        protected override List<ushort> GetFightTargetsIDs()
        {
            ...
            {
                (ushort)ParseEnum.EvtcTargetIDS.Qadim,
        ...
        {
            // Find target
            AgentItem target = agentData.GetAgentsByID((ushort)ParseEnum.EvtcTargetIDS.Qadim).FirstOrDefault();
            if (target == null)
            {
        ...
        public override List<PhaseData> GetPhases(ParsedLog log, bool requirePhases)
        {
            ...
            Target qadim = Targets.Find(x => x.ID == (ushort)ParseEnum.EvtcTargetIDS.Qadim);
            if (qadim == null)
            {
        ...
        public override void ComputeTargetCombatReplayActors(Target target, ParsedLog log, CombatReplay replay)
        {
            ...
            switch (target.ID)
            {
                case (ushort)ParseEnum.EvtcTargetIDS.Qadim:
                    //CC
                    ...
                    {
                        int radius = ccRadius;
                        replay.Actors.Add(new CircleDecoration(true, 0, ccRadius, ((int)c.Time, (int)c.Time + c.ActualDuration), "rgba(0, 180, 255, 0.3)", new AgentConnector(target)));
                    }
                    //Riposte
                    ...
                    {
                        int radius = 2200;
                        replay.Actors.Add(new CircleDecoration(true, 0, radius, ((int)c.Time, (int)c.Time + c.ActualDuration), "rgba(255, 0, 0, 0.5)", new AgentConnector(target)));
                    }
                    //Big Hit
                    ...
                    foreach (AbstractCastEvent c in maceShockwave)
                    {
                        ...
                        if (facing != null && targetPosition != null)
                        {
                            ...
                            replay.Actors.Add(new CircleDecoration(true, 0, impactRadius, (start, start + delay), "rgba(255, 100, 0, 0.2)", new PositionConnector(position)));
                            replay.Actors.Add(new CircleDecoration(true, 0, impactRadius, (start + delay - 10, start + delay + 100), "rgba(255, 100, 0, 0.7)", new PositionConnector(position)));
                            replay.Actors.Add(new CircleDecoration(false, start + delay + duration, radius, (start + delay, start + delay + duration), "rgba(255, 200, 0, 0.7)", new PositionConnector(position)));
                        }
                    }
                    break;
                case (ushort)AncientInvokedHydra:
                    ...
                    foreach (AbstractCastEvent c in eleBreath)
                    {
                        ...
                        if (facing != null)
                        {
                            replay.Actors.Add(new PieDecoration(true, 0, radius, facing, openingAngle, (start + delay, start + delay + duration), "rgba(255, 180, 0, 0.3)", new AgentConnector(target)));
                        }
                    }
                    break;
                case (ushort)WyvernMatriarch:
                    ...
                    foreach (AbstractCastEvent c in wingBuffet)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, range, span, rotation, range / 2, (start, start + preCast), "rgba(0,100,255,0.2)", new AgentConnector(target)));
                            replay.Actors.Add(new RotatedRectangleDecoration(true, 0, range, span, rotation, range / 2, (start + preCast, start + duration), "rgba(0,100,255,0.5)", new AgentConnector(target)));
                        }
                    }
                    //Breath
                    ...
                    foreach (AbstractCastEvent c in matBreath)
                    {
                        ...
                        {
                            replay.Actors.Add(new PieDecoration(true, 0, radius, facing, openingAngle, (start + delay, start + delay + duration), "rgba(255, 200, 0, 0.3)", new AgentConnector(target)));
                            replay.Actors.Add(new PieDecoration(true, 0, radius, facing, openingAngle, (start + delay + duration, start + delay + fieldDuration), "rgba(255, 50, 0, 0.3)", new PositionConnector(pos)));
                        }
                    }
                    //Tail Swipe
                    ...
                    foreach (AbstractCastEvent c in matSwipe)
                    {
                        ...
                        if (facing != null)
                        {
                            ...
                            {
                                int rotation = Point3D.GetRotationFromFacing(facing);
                                replay.Actors.Add(new PieDecoration(false, 0, maxRadius - (i * radiusDecrement), rotation - (i * angleIncrement), openingAngle, (start, start + delay), "rgba(255, 255, 0, 0.6)", new AgentConnector(target)));
                                replay.Actors.Add(new PieDecoration(true, 0, maxRadius - (i * radiusDecrement), rotation - (i * angleIncrement), openingAngle, (start, start + delay), "rgba(255, 180, 0, 0.3)", new AgentConnector(target)));
                ...
                case (ushort)ApocalypseBringer:
                    ...
                    foreach (AbstractCastEvent c in jumpShockwave)
                    {
                        ...
                        int duration = 3000;
                        int maxRadius = 2000;
                        replay.Actors.Add(new CircleDecoration(false, start + delay + duration, maxRadius, (start + delay, start + delay + duration), "rgba(255, 200, 0, 0.5)", new AgentConnector(target)));
                    }
                    ...
                    foreach (AbstractCastEvent c in stompShockwave)
                    {
                        ...
                        {
                            Point3D position = new Point3D(targetPosition.X + facing.X * spellCenterDistance, targetPosition.Y + facing.Y * spellCenterDistance, targetPosition.Z, targetPosition.Time);
                            replay.Actors.Add(new CircleDecoration(true, 0, impactRadius, (start, start + delay), "rgba(255, 100, 0, 0.1)", new PositionConnector(position)));
                            replay.Actors.Add(new CircleDecoration(true, 0, impactRadius, (start + delay - 10, start + delay + 100), "rgba(255, 100, 0, 0.5)", new PositionConnector(position)));
                            replay.Actors.Add(new CircleDecoration(false, start + delay + duration, maxRadius, (start + delay, start + delay + duration), "rgba(255, 200, 0, 0.5)", new PositionConnector(position)));
                        }
                    }
                    //CC
        ...
        {
            Target target = Targets.Find(x => x.ID == (ushort)ParseEnum.EvtcTargetIDS.Qadim);
            if (target == null)
            {
