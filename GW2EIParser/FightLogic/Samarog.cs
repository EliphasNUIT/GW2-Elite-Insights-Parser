...
namespace GW2EIParser.Logic
{
    public class Samarog : RaidLogic
    {
        public Samarog(ushort triggerID) : base(triggerID)
        {
            ...
            new EnemyBoonRemoveMechanic(38226, "Brutalize", new MechanicPlotlySetting("diamond-tall","rgb(0,160,0)"), "CC End","Ended Brutalize", "CC Ended",0),
                ...
                case (ushort)ParseEnum.EvtcNPCIDs.Samarog:
                    ...
                    foreach (AbstractBuffEvent c in brutalize)
                    {
                        ...
                        else
                        {
                            ...
                            replay.Actors.Add(new CircleDecoration(true, 0, 120, (brutStart, brutEnd), "rgba(0, 180, 255, 0.3)", new AgentConnector(target)));
                        }
                    }
                    break;
                case (ushort)Rigom:
        ...
        public override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            ...
            foreach (AbstractBuffEvent c in bigbomb)
            {
                ...
                replay.Actors.Add(new CircleDecoration(true, 0, 300, (bigStart, bigEnd), "rgba(150, 80, 0, 0.2)", new AgentConnector(p)));
                replay.Actors.Add(new CircleDecoration(true, bigEnd, 300, (bigStart, bigEnd), "rgba(150, 80, 0, 0.2)", new AgentConnector(p)));
            }
            // small bomb
            ...
            foreach (AbstractBuffEvent c in smallbomb)
            {
                ...
                replay.Actors.Add(new CircleDecoration(true, 0, 80, (smallStart, smallEnd), "rgba(80, 150, 0, 0.3)", new AgentConnector(p)));
            }
            // fixated
            ...
            foreach (AbstractBuffEvent c in fixatedSam)
            {
                ...
                else
                {
                    ...
                    replay.Actors.Add(new CircleDecoration(true, 0, 80, (fixatedSamStart, fixatedSamEnd), "rgba(255, 80, 255, 0.3)", new AgentConnector(p)));
                }
            }
            //fixated Ghuldem
            ...
            foreach (AbstractBuffEvent c in fixatedGuldhem)
            {
                ...
                else
                {
                    ...
                    if (guldhem != null)
                    {
                        replay.Actors.Add(new LineDecoration(0, (fixationGuldhemStart, fixationGuldhemEnd), "rgba(255, 100, 0, 0.3)", new AgentConnector(p), new AgentConnector(guldhem)));
                    }
                }
            }
            //fixated Rigom
            ...
            foreach (AbstractBuffEvent c in fixatedRigom)
            {
                ...
                else
                {
                    ...
                    if (rigom != null)
                    {
                        replay.Actors.Add(new LineDecoration(0, (fixationRigomStart, fixationRigomEnd), "rgba(255, 0, 0, 0.3)", new AgentConnector(p), new AgentConnector(rigom)));
                    }
                }
            }
        }
