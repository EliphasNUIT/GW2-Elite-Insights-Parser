...
namespace GW2EIParser.Parser.ParsedData
{
    public class CombatData
    {
        ...
        private readonly Dictionary<AgentItem, List<AbstractBuffEvent>> _boonDataBySrc;
        ...
        private void SpecialBoonParse(List<Player> players, SkillData skillData, FightData fightData)
        {
            ...
            {
                _boonDataBySrc[a].Sort((x, y) => x.Time.CompareTo(y.Time));
            }
        }
        ...
        public CombatData(List<CombatItem> allCombatItems, FightData fightData, AgentData agentData, SkillData skillData, List<Player> players)
        {
            ...
            var noStateActiBuffRem = allCombatItems.Where(x => x.IsStateChange == ParseEnum.EvtcStateChange.None && x.IsActivation == ParseEnum.EvtcActivation.None && x.IsBuffRemove == ParseEnum.EvtcBuffRemove.None);
            // movement events
        ...
        public List<AbstractBuffEvent> GetBuffDataBySrc(AgentItem key)
        {
            if (_boonDataBySrc.TryGetValue(key, out List<AbstractBuffEvent> res))
            {
                return res;
            }
            return new List<AbstractBuffEvent>(); ;
